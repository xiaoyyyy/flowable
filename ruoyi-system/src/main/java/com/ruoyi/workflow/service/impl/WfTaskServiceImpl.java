...
public class WfTaskServiceImpl extends FlowServiceFactory implements IWfTaskService {
    ...
    public void complete(WfTaskBo taskBo) {
        ...
        Map<String, String> userTaskProperties = ModelUtils.getUserTaskProperties(bpmnModel, task.getTaskDefinitionKey());
        identityService.setAuthenticatedUserId(TaskUtils.getUserId());
        // 小注意：候选任务在 complete 前最好先 claim/签收。项目 complete 里已经 setAssignee 了，等价于签收，通常没问题；如果遇到"不能完成未签收任务"的报错，就在
        //  complete 前补一个 taskService.claim。

        // ===== 完成前：读取或签组信息（完成后运行时数据会被清理）=====
        String orSignGroup = (String) taskService.getVariableLocal(task.getId(), "orSignGroup");
        // 缓存后续建影子任务需要的字段
        String procInsId = task.getProcessInstanceId();
        String procDefId = task.getProcessDefinitionId();
        String taskDefKey = task.getTaskDefinitionKey();
        String taskName = task.getName();
        String doneUserId = TaskUtils.getUserId();   // 实际办理人
        ...
        if (StringUtils.isNotBlank(orSignGroup)) {
            createShadowTasks(orSignGroup, doneUserId, procInsId, procDefId, taskDefKey, taskName, taskBo.getComment());
        }
    ...
    /**
     * 影子任务创建法：
     * 为或签组中未实际办理的成员补一条“已完成”的影子历史任务，
     * 使其在“已办列表(taskAssignee)”中可见，并挂上审批记录。
     */
    private void createShadowTasks(String orSignGroup, String doneUserId, String procInsId,
                                   String procDefId, String taskDefKey, String taskName, String comment) {
        for (String memberId : orSignGroup.split(",")) {
            if (StringUtils.equals(memberId, doneUserId)) {
                continue; // 跳过实际办理人
            }
            // 建独立任务并关联到流程实例
            TaskEntity shadow = (TaskEntity) taskService.newTask();
            shadow.setName(taskName);
            shadow.setProcessInstanceId(procInsId);
            shadow.setProcessDefinitionId(procDefId);
            shadow.setTaskDefinitionKey(taskDefKey);
            shadow.setAssignee(memberId);
            shadow.setTenantId("AMS");
            taskService.saveTask(shadow);

            // 审批记录：说明是或签由他人完成
            String nickName = sysUserService.selectNickNameById(Long.parseLong(doneUserId));
            String shadowComment = StrUtil.format("或签：由[{}]代表本组审批。{}",
                StringUtils.isNotBlank(nickName) ? nickName : doneUserId,
                StringUtils.isNotBlank(comment) ? "意见：" + comment : "");
            taskService.addComment(shadow.getId(), procInsId, FlowComment.NORMAL.getType(), shadowComment);

            // 立即完成 -> 只进历史表，不参与流程流转
            taskService.complete(shadow.getId());
        }
    }
