...
public class ModelUtils {
    ...
    /**
     * 获取用户任务节点的扩展属性（flowable:properties）键值对
     *
     * @param model   bpmnModel对象
     * @param taskKey 任务Key
     * @return 属性键值对（name -> value）
     */
    public static Map<String, String> getUserTaskProperties(BpmnModel model, String taskKey) {
        ...
        // flowable:properties 为包装元素，其子元素 property 承载具体键值
        List<ExtensionElement> propertiesWrappers = extensionElements.get("properties");
        if (propertiesWrappers == null) {
            return result;
        }
        for (ExtensionElement wrapper : propertiesWrappers) {
            List<ExtensionElement> propertyList = wrapper.getChildElements().get("property");
